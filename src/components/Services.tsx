      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
          {specialEvents.map((event, index) => (
            <div key={index} className="bg-gradient-hero p-8 rounded-2xl text-primary-foreground shadow-elegant">
              <h3 className="text-2xl font-bold mb-2">{event.title}</h3>
              {event.date && <p className="text-lg mb-4 text-gold">{event.date}</p>}
              {event.phone && <p className="text-lg mb-4 text-gold">{event.phone}</p>}
        ...
        <div className="mt-16 text-center bg-secondary/50 rounded-2xl p-8">
          <h3 className="text-2xl font-bold text-primary mb-4">Informations Pratiques</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-muted-foreground">
            <div>
              ...
              <p className="text-sm">Cocody, Attoban derrière le commissariat du 30ème arrondissement, près de la CGRAE, Abidjan</p>
            </div>
            <div>
